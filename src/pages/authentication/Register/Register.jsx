import React, { useEffect, useState } from "react";

import AppButton from "../../../components/shared/AppButton/AppButton";
import * as Yup from "yup";
import { Field, Form, Formik } from "formik";
import FormGroup from "../../../components/shared/FormGroup/FormGroup";
import authService from "../../../services/auth.service";
import { Link, useNavigate } from "react-router-dom";
import Logo from "../../../assets/images/white-logo.png";
import "./Register.scss";
import PinInput from "react-pin-input";
import Select from "react-select";
import { COUNTRIES } from "../../../components/CompleteProfileForm/countries";
import authUtils from "../../../utils/authUtils";
import { ButtonGroup, Dropdown } from "react-bootstrap";
import { Trans, useTranslation } from "react-i18next";
import LangIcon from "../../../assets/icons/language.png";

const registerSchema = Yup.object().shape({
  firstName: Yup.string().required(),
  lastName: Yup.string().required(),
  phone: Yup.number().required(),
  countryResidency: Yup.string().required(),
  email: Yup.string().email().required(),
  portalPassword: Yup.string().required(),
  confirmPassword: Yup.string()
    .required()
    .oneOf([Yup.ref("portalPassword"), null], "Passwords must match"),
  declarations: Yup.array()
    .min(1, "You must tick the consent form")
    .required("You must tick the consent form"),
});

const Register = () => {
  const { t, i18n } = useTranslation();
  const [lang, setlang] = useState(i18n.language);

  const navigate = useNavigate();
  const [loading, setloading] = useState(false);
  const [emailError, setemailError] = useState(null);
  const [phoneError, setphoneError] = useState(null);
  const [pinError, setPinError] = useState(null);
  const [isPinSent, setisPinSent] = useState(false);
  const [isPinValid, setisPinValid] = useState(false);
  const [selectedCountry, setselectedCountry] = useState(
    COUNTRIES.find((c) => c?.code == "AE")
  );

  const handleSubmit = async (values) => {
    setloading(true);
    resetErrors();
    const isEmailValid = await authUtils.checkEmailAvailability(values.email);
    if (!isEmailValid) {
      setemailError("Email already exists");
      setloading(false);

      return;
    }
    const phone = `+${selectedCountry.calling_code}${values.phone}`;
    const isPhoneValid = await authUtils.checkPhoneAvailability(phone);
    if (!isPhoneValid) {
      setphoneError("Phone already exists");
      setloading(false);
      return;
    }
    try {
      const isSent = await authUtils.sendPinToEmail(values.email);
      if (isSent) {
        const data = { ...values };
        data.type = "individual";
        delete data.confirmPassword;
        data.phone = phone;
        navigate("/verify-pin", { state: values });
      } else {
        setPinError("Your limit exceeded, try again later");
      }
    } catch (error) {
      console.log(error);
    }
    setloading(false);
  };

  const handleSendPin = async (email) => {
    try {
      resetErrors();
      const isEmailValid = await authUtils.checkEmailAvailability(email);
      if (!isEmailValid) {
        setemailError("Email already exists");
        return;
      }
      const isPinSent = await authUtils.sendPinToEmail(email);
      if (isPinSent) {
        setisPinSent(true);
      }
    } catch (error) {
      console.log(error);
    }
  };

  const resetErrors = () => {
    if (emailError) setemailError(null);
    if (phoneError) setphoneError(null);
    if (pinError) setPinError(null);
  };

  useEffect(() => {
    document.documentElement.dir = i18n.dir();
    document.documentElement.lang = i18n.language;
  }, []);

  const changeLanguage = (lng) => {
    i18n.changeLanguage(lng);
    setlang(lng)
  };

  return (
    <div className="register-wrapper">
      <div>
        <div className="text-center">
          <img src={Logo} alt="Accuindex logo" width={115} />
          <h5>{t('register.title')}</h5>
          <p style={{ maxWidth: "70%" }} className="text-center mx-auto mt-3">{t('register.sub-title')}</p>
        </div>
      </div>
      <div className="card-wrapper">
        <section className="change-lang">
          <Dropdown as={ButtonGroup} onSelect={(lang) => changeLanguage(lang)}>
            <Dropdown.Toggle
              id="dropdown-custom-1"
              value={lang}
              className="bg-transparent border text-dark fs-6 ps-3"
            >
              <img src={LangIcon} alt="" height={20} className="me-1" />
              <span className="pe-3"> {langMap[lang]}</span>
            </Dropdown.Toggle>
            <Dropdown.Menu className="super-colors">
              <Dropdown.Item eventKey="en-US">English</Dropdown.Item>
              <Dropdown.Item eventKey="vi">TIẾNG VIỆT NAM</Dropdown.Item>

              {/* <Dropdown.Item
                eventKey="ar"
                style={{ fontFamily: "Noto Kufi Arabic" }}
              >
                العربية
              </Dropdown.Item>
              <Dropdown.Item eventKey="ch">Chinese</Dropdown.Item>
              <Dropdown.Item eventKey="fa">فارسی</Dropdown.Item> */}
            </Dropdown.Menu>
          </Dropdown>
        </section>
        <img
          src="data:image/png;base64,"
          alt=""
          style={{ maxHeight: 130 }}
          className="mx-auto mb-5 d-lg-none"
        />

        <div>

          <h5 className="text-primary fw-bold">{t('register.heading')}</h5>
          <small
            className="text-muted d-block mb-3 mt-2 mb-4"
            style={{ maxWidth: "80%" }}>
            {t('register.description')}
          </small>
          <Formik
            initialValues={{
              firstName: "",
              lastName: "",
              phone: "",
              countryResidency: selectedCountry.countryEn,
              email: "",
              portalPassword: "",
              confirmPassword: "",
              declarations: [],
            }}
            validateOnChange={false}
            validationSchema={registerSchema}
            onSubmit={(values) => {
              handleSubmit(values);
            }}
          >
            {({ errors, handleChange, setFieldValue, values }) => (
              <Form>
                <div className="row g-2">
                  <div className="col-12 col-lg-6">
                    <FormGroup
                      placeholder={t('register.form.labels.firstName')}
                      className="py-3"
                      name="firstName"
                      onChange={handleChange}
                      errors={errors}
                    />
                  </div>
                  <div className="col-12 col-lg-6">
                    <FormGroup
                      placeholder={t('register.form.labels.lastName')}
                      className="py-3"
                      name="lastName"
                      onChange={handleChange}
                      errors={errors}
                    />
                  </div>
                  <div className="col-12 mb-2">
                    <div>
                      <Select
                        className="react-select-container"
                        classNamePrefix="react-select"
                        placeholder=""
                        name="countryResidency"
                        options={COUNTRIES}
                        defaultValue={selectedCountry}
                        onChange={(e) => {
                          console.log(e);
                          setselectedCountry(e);
                          setFieldValue("countryResidency", e.countryEn);
                        }}
                        getOptionValue={(op) => op.countryEn}
                        getOptionLabel={(op) => op[lang == 'ar' ? 'countryAr' : 'countryEn']}
                        theme={(t) => ({
                          ...t,
                          borderRadius: 0,
                          borderColor: "red",
                        })}
                      />
                      {errors.countryResidency && (
                        <small className="text-danger small error-message">
                          {errors.countryResidency}
                        </small>
                      )}
                    </div>
                  </div>
                  <div className="col-12 col-lg-6 mb-2">
                    <div>
                      <div className="input-group mb-2">
                        <span
                          className="input-group-text bg-transparent"
                          id="basic-addon1"
                        >
                          +{selectedCountry.calling_code}
                        </span>

                        <input
                          type="text"
                          className="form-control flex-grow-1 py-3"
                          aria-label="Text input with dropdown button"
                          placeholder={t('register.form.labels.phone_number')}
                          name="phone"
                          onChange={handleChange}
                        />
                      </div>
                      {errors.phone ||
                        (phoneError && (
                          <small className="text-danger small error-message">
                            {errors.phone || phoneError}
                          </small>
                        ))}
                    </div>
                  </div>
                  <div className="col-12 col-lg-6">
                    <FormGroup
                      placeholder={t('register.form.labels.email')}
                      className="py-3"
                      name="email"
                      onChange={handleChange}
                      errors={errors}
                      customError={emailError}
                    />
                  </div>
                  <div className="col-12 col-lg-6">
                    <FormGroup
                      placeholder={t('register.form.labels.password')}
                      type="password"
                      className="py-3"
                      name="portalPassword"
                      onChange={handleChange}
                      errors={errors}
                    />
                  </div>
                  <div className="col-12 col-lg-6">
                    <FormGroup
                      placeholder={t('register.form.labels.confirm_password')}
                      type="password"
                      className="py-3"
                      name="confirmPassword"
                      onChange={handleChange}
                      errors={errors}
                    />
                  </div>
                </div>
                <div className="mb-3">
                  <div className="form-check my-3">
                    <input
                      className="form-check-input"
                      type="checkbox"
                      name="declarations"
                      value="By clicking here I give my consent for Accuindex to contact me for marketing purposes. You can opt out at any time. For further details please see ourMarketing and Communication Policy Statement."
                      id="flexCheckDefault"
                      onChange={handleChange}
                    />
                    <Trans i18nKey="register.form.agreement">
                      By registering you agree to our <a className="text-primary fw-semibold text-decoration-none" href="#">privacy policy</a>
                    </Trans>

                  </div>
                  {errors.declarations && (
                    <small className="text-danger small error-message d-block">
                      {errors.declarations}
                    </small>
                  )}
                </div>
                {pinError && (
                  <div className="alert alert-danger" role="alert">
                    {pinError}
                  </div>
                )}
                <AppButton
                  type="submit"
                  className="btn btn-primary px-5 mt-3"
                  loading={loading}
                >
                  {t('register.form.cta')}
                </AppButton>
              </Form>
            )}
          </Formik>
          <Trans i18nKey="register.already_have_account">
            <div className="mt-4 small">
              Already have an account?{" "}
              <Link to="/login" className="fw-bold text-primary text-decoration-none">
                Log in
              </Link>
            </div>
          </Trans>

        </div>
      </div>
    </div>
  );
};

export default Register;

const langMap = {
  ar: "العربية",
  ["en-US"]: "English",
  vi: 'Vietnamese',
  ch: 'Chinese',
  fa: 'فارسی'

};