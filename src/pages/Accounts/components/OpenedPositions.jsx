...
const OpenedPositions = ({ selectedAccountId }) => {
  ...
  useEffect(() => {
    (async function () {
      setloading(true);
      try {
        const { result } = await tradingAccountsService.getAccountOpenPositions(
          selectedAccountId,
          query
        );
  ...
  return (
    <section className="section">
      <h5 className="heading"> {t("trading_accounts.positions.open_title")}</h5>
            ...
            {!loading && data.length == 0 && (
              <tr>
                <td colSpan={10} className="text-md-center">
                  {t("trading_accounts.positions.table_body.no_open_data")}
                </td>
              </tr>
            )}
            {!loading &&
              data.map((position) => (
                <tr>
                  ...
                  <td>{t("trading_accounts.positions.table_body.buy")}</td>
